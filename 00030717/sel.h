...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,ka,delta,lambda,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G-L ]
    // [  D    0  ]
    Matrix matrixA,matrixK,matrixG,matrixD, matrixL;
    Matrix K,g_matrix,g_matrix_t,Alpha,Beta,Alphat,Betat,BPrima,BPrimat;

    //Preparando matrixB (En clase conocida simplemente como A)
    ...
    ka = m.getParameter(KA);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix(ka*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK);

    //Preparando matrixG (En clase conocida simplemente como G)
    lambda = m.getParameter(LAMBDA);
    calculateBPrima(BPrima);
    productRealMatrix((lambda*J)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
...
Vector createLocalb(int e,mesh &m){
    ...
    float p_x = m.getParameter(EXTERNAL_FORCE_X);
    float p_y = m.getParameter(EXTERNAL_FORCE_Y);
